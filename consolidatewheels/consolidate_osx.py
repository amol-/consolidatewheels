    ...
    for wheeldir in wheeldirs:
        for lib_to_patch_path in pathlib.Path(wheeldir).rglob(".dylibs/*"):
            ...
            if libname not in patched_identifier:
                ...
                patched_identifier[libname] = libid = os.path.join("/CLD", f"{consolidated_id}", libname)
        ...
        for lib_to_patch_path in pathlib.Path(wheeldir).rglob("*.so"):
            ...
            # print("\t\tDEPENDENCIES FOR", lib_to_patch_path, dependencies)
            for dependency, dependency_path in dependencies.items():
                ...
                if dependency not in seen_dependencies:
                    ...
                    continue
                update_dependency_path(lib_to_patch_path, dependency_path, patched_identifier[dependency])
            resign_library(lib_to_patch_path)
            # print("\t\tUPDATED DEPS FOR", lib_to_patch_path, get_library_dependencies(lib_to_patch_path))
